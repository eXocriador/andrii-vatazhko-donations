...
const drafts = [
  { id: 'boats-next', title: 'Катери для Берислава', goal: 540000, deadline: '10 кві 2025' },
  {
    id: 'stabilization',
...
const workflow = [
  { title: 'Заявка', description: 'Військові або лікарі залишають запит через форму або Telegram.' },
  { title: 'Верифікація', description: 'Координатори підтверджують запит, формують кошторис та дедлайн.' },
  { title: 'Збір', description: 'Публікуємо кампанію, оновлення й push у соцмережах.' },
  { title: 'Звіт', description: 'Фото/відео із передачі та фінальний акт у PDF.' },
]
  ...
  return (
    <section className={`${styles.page} pageShell`}>
      <header className={styles.header}>
        ...
        <h1>Адмін-панель</h1>
        <p>
          Тут зʼявиться внутрішня CRM для керування зборами, аналітики по банці та швидкого публікування
          звітів. Поки що це лише попередній макет.
        </p>
      </header>
          ...
          <div className={styles.reports}>
            {recentReports.map((report) => (
              <div key={report.id} className={styles.reportItem}>
                ...
                <span className={styles.tag}>{report.tags[0]}</span>
              </div>
            ))}
          </div>
        </article>
      ...
      <div className={`uiCard ${styles.callout}`}>
        <div>
          ...
          <p>Плануємо підтримати авторизацію через email-код та Telegram. Слідкуй за апдейтами.</p>
        </div>
